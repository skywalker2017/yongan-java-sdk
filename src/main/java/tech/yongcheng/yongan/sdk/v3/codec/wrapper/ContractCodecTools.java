...
public class ContractCodecTools {
    ...
    private static byte[] typeEncoderWrapper(Type parameter, boolean isWasm) throws IOException {
        return isWasm
                ...
                : tech.yongcheng.yongan.sdk.v3.codec.abi.TypeEncoder.encode(parameter);
    }
                        ...
                        case BOOL:
                            {
                                abiObject.setBoolValue(
                                        tech.yongcheng.yongan.sdk.v3.codec.abi.TypeDecoder.decode(
            ...
            case LIST:
                {
                    ...
                    if (abiObject.getListType() == ABIObject.ListType.DYNAMIC) {
                        // dynamic list length
                        listLength =
                                TypeDecoder.decode(reader, TypeReference.create(Uint256.class))
                                        .getValue()
                                        .intValue();
                    } else {
                        // fixed list length
